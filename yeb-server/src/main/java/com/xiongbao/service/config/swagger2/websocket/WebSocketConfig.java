package com.xiongbao.service.config.swagger2.websocket;
...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    /*
     *  添加这个Endpoints,这样就可以通过websocket连接上服务
     *  也就是配置websocket 的服务地址,并且可以指定是否使用socketJS
     * */
    @Override
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        /*
         *   1.将ws/ep路径注册为stomp,用户连接了这个端点就可以进行websocket通讯, 支持socketJS
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        // 配置代理域,可以配置多个, 配置代理目的地前缀为/queue,可以配置域上向客户端推送消息
